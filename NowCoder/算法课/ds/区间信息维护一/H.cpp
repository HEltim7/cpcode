...
void solve() {
    int n,m;
    cin>>n>>m;
    for(int i=0;i<3;i++) fill(dif[i]+1, dif[i]+n+1, 0);
    while(m--) {
        int type,pos;
        cin>>type>>pos;
        if(type==1) dif[0][pos]++;
        else if(type==2) dif[1][pos]++;
        else dif[2][pos]++,dif[2][pos+1]++;
    }
    for(int i=1;i<=n;i++) dif[2][i]+=dif[2][i-1];
    for(int i=1;i<=n;i++) dif[1][i]+=dif[1][i-1]+dif[2][i];
    for(int i=1;i<=n;i++) dif[0][i]+=dif[0][i-1]+dif[1][i];

    for(int i=1;i<=n;i++) cout<<dif[0][i]<<" \n"[i==n];
}
