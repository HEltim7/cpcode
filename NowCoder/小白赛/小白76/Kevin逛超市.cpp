...
Mint solve(int l,int r,int deep) {
    int len=r-l+1;
    if(mp.find(len)!=mp.end()) return mp[len]+Mint(deep)*len;

    Mint res;
    if(len==1) res=0;
    else if(len==1<<__lg(len)) res=Mint(len)*(__lg(len)-inv2);
    else {
        int mid=(l+r)/2;
        res=solve(l,mid,1)+solve(mid+1,r,1);
    }
    mp[len]=res;
    return res+Mint(deep)*len;
}
