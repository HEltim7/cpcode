...
constexpr int M=1e6+10,N=4.2e3+10;
int prime[M],idx;
bool isnp[M];
Mint dp[N][N];
int cnt[N],cnt_pre[N];
...
void solve() {
    ...
    int dif=0;
    for(auto [_,x]:p) cnt[++dif]=x;
    for(int i=1;i<=dif;i++) cnt_pre[i]=cnt_pre[i-1]+cnt[i];
    ...
    for(int i=1;i<=dif;i++) {
        for(int j=0;j<=min(i,n);j++) {
            if(j<i) {
                int tot=cnt_pre[i-1]-j+1;
                dp[i][j]+=dp[i-1][j]*cmb(tot+cnt[i]-1,tot-1);
            }
            if(j) {
                int tot=cnt_pre[i-1]-(j-1)+1;
                dp[i][j]+=dp[i-1][j-1]*cmb(tot+(cnt[i]-1)-1,tot-1);
            }
        }
    }

    int tot=cnt_pre[dif]-n+1;
    Mint ans=dp[dif][n];
    for(auto [_,x]:np) {
        ans*=cmb(tot+x-1,tot-1);
        tot+=x;
    }
    cout<<ans;
}
