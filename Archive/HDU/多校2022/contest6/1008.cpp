...
PII build(int bit,int l,int r,LL p) {
    if(bit<0) return {l,r};
    int mid=l+r>>1;
    if(p&(1<<bit)) return build(bit-1,mid+1,r,p);
    return build(bit-1,l,mid,p);
}

void solve() {
    LL n=read(n),x=read(x);
    int bit=__lg(x)-1;
    PII rng=build(bit,1,n,x);
    debug(bit,build(bit,1,n,x));
    int l=rng.first,r=rng.second;
    LL ans=0;
    LL len=r-l+1;
    debug(len);
    ans=(1+len)*len/2+len*(n-r);
