...
struct Info {
    ...
    void pushup(const Info &l,const Info &r) {
        vector v({pair(l.mint,l.minid),pair(r.mint,r.minid),pair(t,id)});
        sort(v.begin(),v.end());
        tie(mint,minid)=v.front();
    }
...
void solve() {
    ...
    auto edge=[&](int u,int v,int w,int t) {
        if(lct.same(u, v)) {
            int rt=lct.split(u, v);
            int id=lct.info(rt).minid;
            int tt=lct.info(rt).mint;
            auto [x,y,z]=edg[id];
            if(tt>t) {
                lct.cut(x, z);
                lct.cut(y, z);
